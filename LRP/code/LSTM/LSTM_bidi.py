...
import numpy as np
import pickle
from LRP.code.LSTM.LRP_linear_layer import *
import torchfile
...
class LSTM_bidi:
    ...
    def __init__(self, model_path='./model/'):
        ...
        """
        torch_model = torchfile.load(model_path)

        # vocabulary
        f_voc     = open('./data/vocab', 'rb')
        self.voc  = pickle.load(f_voc)
        f_voc.close()
        
        # word embeddings
        self.E    = np.array(torch_model[0][0]) # shape V*e
        
        # model weights
        # LSTM left encoder
        self.Wxh_Left  = np.array(torch_model[0][1])  # shape 4d*e
        self.bxh_Left  = np.array(torch_model[0][2])  # shape 4d 
        self.Whh_Left  = np.array(torch_model[0][3])  # shape 4d*d
        self.bhh_Left  = np.array(torch_model[0][4])  # shape 4d  
        # LSTM right encoder
        self.Wxh_Right = np.array(torch_model[1][1])
        self.bxh_Right = np.array(torch_model[1][2])
        self.Whh_Right = np.array(torch_model[1][3])
        self.bhh_Right = np.array(torch_model[1][4]) 
        # linear output layer
        self.Why_Left  = np.array(torch_model[2][0])   # shape C*d
        self.Why_Right = np.array(torch_model[2][1])  # shape C*d
